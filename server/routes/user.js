...
router.post("/updatePassword", function (req, res) {
    ...
    var reNewPassword = md53.update(req.body.reNewPassword).digest('base64');
    var id = req.session.user.id;
    User.getUserById(id, function (err, result) {
        if (result.password != oldPassword) return res.send({ "error": 403, "errorCode":"1","message": "密码错误!" });
        if (newPassword != reNewPassword) return res.send({ "error": 403, "errorCode":"2","message": "新密码和确认密码不一致!" });
